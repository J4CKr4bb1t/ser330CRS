...
//THIS PAGE HAS BEEN REFACTORED BY SESE
...
class Institution {
  ...
  //REFACTORED to return the list of students in addition to logging it
  listStudents() {
    console.log(
  ...
  //REFACTORED for testing - added a return
  enroll_student(student) {
    ...
    //added return to verify enrollement, should return in success or failure
    return this.studentList;
  }
  ...
  //REFACTORED also returns list of instructors
  list_instructors() {
    console.log(
      `\nInstructor List (${this.name})\n-------------------------------------------`
    );
    const facultyList = Object.values(this.facultyList).map(
  ...
  hire_instructor(instructor) {
    if (instructor instanceof Instructor) {
      if (instructor.username in this.facultyList) {
        console.log(
          `${instructor.firstName} ${instructor.lastName} already works at this institution!`
        );
      } else {
    ...
    for (const offering of this.courseSchedule[courseName]) {
      if (
        ...
        if (offering.instructor === instructor) {
          console.log(
            `${instructor.first_name} ${instructor.last_name} is already teaching this course`
          );
        } else {
          offering.instructor = instructor;
          instructor.courseList.push(offering);
          console.log(
            `${instructor.firstName} ${instructor.lastName} has been assigned to teach ${offering}`
          );
        }
      } else {
        console.log(
  ...
  list_course_schedule(year, quarter, dept = null) {
    if (dept) {
      ...
      for (const offerings of Object.values(this.courseSchedule)) {
        const filtered = offerings.filter(
          (offering) =>
            ...
            offering.course.department === dept
        );
      ...
      if (Object.keys(this.courseSchedule).length) {
        for (const offerings of Object.values(this.courseSchedule)) {
          const filtered = offerings.filter(
            (offering) => offering.year === year && offering.quarter === quarter
          );
          filtered.forEach((item) => schedule.push(item.toString()));
        }
        if (schedule.length) {
  ...
  list_registered_students(
    ...
    for (const offering of this.courseSchedule[courseName]) {
      if (
        ...
        console.log(
          `Registered Students List (${offering})\n------------------------------------------------------------`
        );
        for (const student of offering.registered_students) {
          console.log(`${student.lastName}, ${student.firstName}`);
        }
        //added return in sucess
        return offering.registered_students;
      }
    }
    //added return on failure
    return "Course Offering Doesn't Exist";
  }
